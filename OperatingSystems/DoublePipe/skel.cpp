...
void computeHash(const string& hashProgName)
{
  ...
  // closing Child'd read end of PC pipe because we are done reading from it
  ...
  */
  FILE* programOutput = popen(cmdLine.c_str(), "r");
  ...
  if(fread(hashValue, sizeof(char), sizeof(char) * MAX_FILE_NAME_LENGTH, programOutput) < 0) {
    perror("fread");
    exit(-1);
  }
  //  close the file pointer FILE*
  if(pclose(programOutput) < 0) {
    perror("pclose");
    exit(-1);
  }
  ...
  */
  ...
  //  closing wrte end of CP pipe unblocks parent's read() function
...
void parentFunc(const string& hashProgName)
{
  ...
  // PC pipe: close the parent READ end
  if(close(parentToChildPipe[READ_END]) < 0) {
    perror("PCR close");
    exit(-1);
  }
  // CP pipe: close the parent WRITE end
  if(close(childToParentPipe[WRITE_END]) < 0) {
    perror("CPW close");
    exit(-1);
  }
  ...
  // write to child through PC pipe
  if(write(parentToChildPipe[WRITE_END], fileName.c_str(), sizeof(fileName)) < 0) {
    perror("PC write");
    exit(-1);
  }
  // close PC pipe WRITE end to unblock child read() function
  ...
  //closing CP Pipe read end because are done reading from it for now
...
int main(int argc, char** argv)
{
  ...
  for (int hashAlgNum = 0; hashAlgNum < HASH_PROG_ARRAY_SIZE; ++hashAlgNum)
  {
    ...
    {
      /** TODO: close the unused ends of two pipes **/
      // PC pipe: close Child WRITE end
      if(close(parentToChildPipe[WRITE_END]) < 0) {
        perror("PCW close");
        exit(-1);
      }
      // CP pipe: close Child READ end
      if(close(childToParentPipe[READ_END]) < 0) {
        perror("CPR close");
        exit(-1);
      }
